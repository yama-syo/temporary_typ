...
function initCV(){
    return new Promise( (res) => {
        const f = () => {
            console.log("opencv_setOK");document.getElementById("opencvRun").firstChild.textContent = "opencv ready";document.getElementById("buildInfo").textContent= cv.getBuildInformation();// console.log("opencv_OK");
        };
...
export async function getKeyInfo(src, canArr) {
    ...
    //キーボードのキーの位置の検出後の画像
    const keyInfo = await findContours(matSrc, sEdgeMat, ctxGA, keySA);
    //opencvでの変数のメモリ開放
    matSrc.delete();edgeMat.delete();sEdgeMat.delete();
...
async function findContours(matSrc, sEdgeMat, ctxGA, keySA) {
    const contours1 = new cv.MatVector();const hierarchy = new cv.Mat();const contours = new cv.MatVector();
    ...
    const obj =  await draw_contours(matSrc, contours, ctxGA, keySA);
    ...
    return (async() =>{
        const keyCtx = keyS.getContext("2d");
        let k = getSkey();
        ...
        await worker.load();
        //全体：
        let sysKey = k.skeyChar;
        if(k.skipKeyPart) sysKey = k.allKeyChar;
        ...
        for (let i = 0; i < obj.b.a.length; i++){
            keyS.width = obj.k[i].width * keyZoom;keyS.height = obj.k[i].height * keyZoom;
            keyCtx.drawImage(await createImageBitmap(obj.b.a[i]), 0, 0);
            // https://github.com/naptha/tesseract.js/blob/master/docs/image-format.md
            const {data: {text}} = await worker.recognize(obj.b.a[i]);
            let t = (await text).toString().replaceAll(/[\n ]/g, "");
            console.log(t.length);
            console.log("detect@" + i + ":" + t);
            if(t.length > 1 && !k.skipKeyPart) {
                const f = k.skeyReg.findIndex( v => {
                    // console.log(t, v.test(t));
                    return (v.test(t));
                });
                // console.log("match:" + f + " / " + i + ":" + t);
                if(f != -1) {
                    console.log("match:" + i + ":" + t);
                    if(f < k.skeyIndex.length){
                        obj.k[i].keys.key = k.skeyIndex[f];
                    }else{
                        obj.k[i].keys.key = t;
                    }
                    obj.k[i].keys.shift = null;
                    obj.k[i].keys.ime = null;
                }
            }else{
                if("A"<= t && t <= "Z") {
                    obj.k[i].keys.shift = t;
                    obj.k[i].keys.key = t.toLowerCase();
                }else{
                    obj.k[i].keys.key = t;
                }
            }
        }
        if(k.skipLang) return obj.k;
        //全体日本語：
        await worker.loadLanguage('jpn');await worker.initialize('jpn');
        await worker.setParameters({
            // tessedit_pageseg_mode: PSM.AUTO,
        ...
        for (let i = 0; i < obj.b.i.length; i++){
            keyS.width = obj.k[i].width * keyZoom;keyS.height = obj.k[i].height * keyZoom;
            const {data: {text}} = await worker.recognize(obj.b.a[i]);
            let t = (await text).toString().replaceAll(/[\n ]/g, "");
            console.log("detect@" + i + ":" + t);
            if(t.length > 1 && obj.k[i].keys.shift != null) {
                const f = k.skeyLangReg.findIndex( v => v.test(t));
                if(f != -1) {
                    console.log("match:" + i + ":" + t);
                    if(f < k.skeyLangIndex.length){
                        obj.k[i].keys.key = k.skeyLangIndex[f];
                    }else{
                        obj.k[i].keys.key = t;
                    }
                    obj.k[i].keys.shift = null;
        ...
        if(k.skipKeyPart) return obj.k;
        //左下
        await worker.loadLanguage('eng');await worker.initialize('eng');
        await worker.setParameters({
            tessedit_pageseg_mode: PSM.SINGLE_CHAR,
            tessedit_char_whitelist: k.keyChar,
        });
        for (let i = 0; i < obj.b.k.length; i++){
            keyS.width = obj.k[i].width * keyZoom;keyS.height = obj.k[i].height * keyZoom;
            // keyCtx.drawImage(await createImageBitmap(obj.b.k[i]), 0, 0);
            const {data: {text}} = await worker.recognize(obj.b.k[i]);
            let t = (await text).toString().replace("\n", "");
            console.log("detect@" + i + ":" + t);
            if(t.length == 1 && obj.k[i].keys.shift != null) {
                console.log("OK@" + i + ":" + t);
                obj.k[i].keys.key = t;
        ...
        for( let i = 0; i < obj.b.s.length; i++ ){
            keyS.width = obj.k[i].width * keyZoom;keyS.height = obj.k[i].height * keyZoom;
            // keyCtx.drawImage(await createImageBitmap(obj.b.s[i]), 0, 0);
            const {data: {text}} = await worker.recognize(obj.b.s[i]);
            let t = (await text).toString().replace("\n", "");
            console.log("detect@" + i + ":" + t);
            if(t.length == 1 && obj.k[i].keys.shift != null) {
                console.log("OK@" + i + ":" + t);
                obj.k[i].keys.shift = t;
                if("A"<= t && t <= "Z")obj.k[i].keys.key = t.toLowerCase();
            }
        }
        //右下
        await worker.loadLanguage('jpn');await worker.initialize('jpn');
        await worker.setParameters({
            tessedit_pageseg_mode: PSM.SINGLE_CHAR,
            tessedit_char_whitelist: k.imeKeyChar,
        });
        for (let i = 0; i < obj.b.i.length; i++){
            keyS.width = obj.k[i].width * keyZoom;keyS.height = obj.k[i].height * keyZoom;
            // keyCtx.drawImage(await createImageBitmap(obj.b.i[i]), 0, 0);
            const {data: {text}} = await worker.recognize(obj.b.i[i]);
            let t = (await text).toString().replace("\n", "");
            console.log("detect@" + i + ":" + t);
            if(t.length == 1  && obj.k[i].keys.ime != null) {
                console.log("OK@" + i + ":" + t);
                obj.k[i].keys.ime = t;
            }
        }
...
async function draw_contours(matSrc, contours, ctxGA, keySA) {
    const keyZ = [];
    let keyInfo = {};
    let blobs = {};blobs.a = [];blobs.s = [];blobs.k = [];blobs.i = [];

    let usedCanvasIndex = -1;
    ...
    for (let i = 0; i < contours.size(); ++i) {
        ...
        if(usingCanvas.length >= keySA.length){
            usedCanvasIndex = await Promise.race(usingCanvas);
        }else usedCanvasIndex++;

        const k = getSkey();
        usingCanvas[usedCanvasIndex] = (async(usedIndex) => {
            //画像から図形の範囲を切り出す
            ...
            //キー画像全体:使用したい言語で検出
            //FIX 日本語、英語以外も対応する？？
            await ocrW[usedIndex].loadLanguage('jpn');await ocrW[usedIndex].initialize('jpn');
            ...
            let t = (await tesseractOCR(ocrW[usedIndex],keySA[usedIndex])).data.text.toString().replaceAll(/[\n ]/g, "");
            console.log("detect@" + i + ":" + t);
            if(t.length > 1) {
                const f = k.skeyLangIndex.findIndex( v => v[0].test(t));
                //キーにある文字列に近い値を検出する正規表現に一致したらf>=0以上
                if(f != -1) {
                    console.log("match:" + i + ":" + t);
                    //検出した文字列をキーにある文字列へ変換するならskeyLangIndexは変換後の文字列を[1]に持つ
                    if(k.skeyLangIndex[f].length == 2){
                        keyInfo[i].keys.key = k.skeyLangIndex[f][1];
                    }else{
                        keyInfo[i].keys.key = t;
                    }
                    keyInfo[i].keys.shift = null;
                    keyInfo[i].keys.ime = null;
                }
            }else{
                //何も文字が検出できなかったフラグ代わりに使う
                keyInfo[i].keys.key = null;
            }

            // 現在のキー画像から文字を検出できないなら次のキー画像へ
            if(keyInfo[i].keys.key == null) return usedIndex;

            //キー画像全体：英語のみで検出
            //FIX OCRを変更できるように(現在:tesseractOCR, そのうち?:google drive api OCR, winrt OCR)
            //TesseractOCR部分
            let sysKey = k.skeyChar;//システムキーのみに使われる文字をWhiteListに
            if(k.skipKeyPart) sysKey = k.allKeyChar;//キー画像を上下左右4分割して見ないならキーボードで使われる文字をWhiteListに
            // await worker.loadLanguage('eng+jpn');await worker.initialize('eng+jpn');
            // await worker.loadLanguage('jpn');await worker.initialize('jpn');
            // await worker.loadLanguage('eng+osd');await worker.initialize('eng+osd');
            await worker.loadLanguage('eng');await worker.initialize('eng');
            ...
            // https://github.com/naptha/tesseract.js/blob/master/docs/image-format.md
            t = (await tesseractOCR(ocrW[usedIndex], keySA[usedIndex])).data.text.toString().replaceAll(/[\n ]/g, "");
            console.log(t.length);
            console.log("detect@" + i + ":" + t);
            if(t.length > 1 && !k.skipKeyPart) {
                const f = k.skeyIndex.findIndex( v => {
                    // console.log(t, v.test(t));
                    return (v[0].test(t));
                });
                // console.log("match:" + f + " / " + i + ":" + t);
                if(f != -1) {
                    ...
                    //システムキーを検出していれば(keys.shiftはnull)キーの検出は終了、次の文字へ
                    return usedIndex;
                }
            }else{
                if("A"<= t && t <= "Z") {
                    keyInfo[i].keys.shift = t;
                    keyInfo[i].keys.key = t.toLowerCase();
                }else{
                    keyInfo[i].keys.key = t;
                }
            }

            //キー画像を上下左右4分割して見ないなら次のキー画像へ
            if(k.skipKeyPart) return usedIndex;
            ...
            //四分割したキー画像の左下//日本語キーボードの場合:キーダウン時の1文字の英数字記号キーの検出(シフト時のキー、下になければ小文字化)
            ...
            if(t.length == 1) {
                console.log("OK@" + i + ":" + t);
                keyInfo[i].keys.shift = t;
                if("A"<= t && t <= "Z")keyInfo[i].keys.key = t.toLowerCase();
            }
            ...
            if(t.length == 1) {
                console.log("OK@" + i + ":" + t);
                keyInfo[i].keySA[usedIndex].ime = t;
            }
            return usedIndex;
        })(usedCanvasIndex);

    //     //画像から図形の範囲を切り出し保存
    //     // https://forum.ionicframework.com/t/dectect-rectangle-and-crop-image-by-opencv-js/145570/3
    //     // https://qiita.com/yokra9/items/f9e98a9b47fe2d1234b0#%E5%AE%9F%E8%A1%8C%E7%B5%90%E6%9E%9Cwasmopencvjs
    //     keyS.height = rect.height * keyZoom;keyS.width = rect.width * keyZoom;
    //     //全体
    //     let re = new cv.Rect(rect.x + offset, rect.y + offset, rect.width - offset * 2, rect.height - offset * 2);
    //     if(keyZoom == 1) cv.imshow(keyS, matSrc.roi(re));
    //     else{
    //         cv.resize(matSrc.roi(re), keyZ, new cv.Size(keyS.width, keyS.height), 0, 0, cv.INTER_CUBIC);
    //         cv.imshow(keyS, keyZ);
    //     }
    //     blobs.a.push(await new Promise(res => keyS.toBlob(res)));

    //     //左上:シフト時のキー、下になければ小文字化
    //     if(keyZoom == 1) cv.imshow(keyS, matSrc.roi(new cv.Rect(re.x, re.y, re.width / 2, re.height / 2)));
    //     else{
    //         cv.resize(matSrc.roi(new cv.Rect(re.x, re.y, re.width / 2, re.height / 2)), keyZ, new cv.Size(keyS.width, keyS.height), 0, 0, cv.INTER_CUBIC);
    //         cv.imshow(keyS, keyZ);
    //     }
    //     blobs.s.push(await new Promise(res => keyS.toBlob(res)));
    //     //左下:キーそのまま
    //     if(keyZoom == 1) cv.imshow(keyS, matSrc.roi(new cv.Rect(re.x, re.y  + re.height / 2, re.width / 2, re.height / 2)));
    //     else{
    //         cv.resize(matSrc.roi(new cv.Rect(re.x, re.y  + re.height / 2, re.width / 2, re.height / 2)), keyZ, new cv.Size(keyS.width, keyS.height), 0, 0, cv.INTER_CUBIC);
    //         cv.imshow(keyS, keyZ);
    //     }
    //     blobs.k.push(await new Promise(res => keyS.toBlob(res)));
    //     //右下:IMEかな入力時キーそのまま
    //     if(keyZoom == 1) cv.imshow(keyS, matSrc.roi(new cv.Rect(re.x + re.width / 2, re.y + re.height / 2, re.width / 2, re.height / 2)));
    //     else{
    //         cv.resize(matSrc.roi(new cv.Rect(re.x + re.width / 2, re.y + re.height / 2, re.width / 2, re.height / 2)), keyZ, new cv.Size(keyS.width, keyS.height), 0, 0, cv.INTER_CUBIC);
    //         cv.imshow(keyS, keyZ);
    //     }
    //     blobs.i.push(await new Promise(res => keyS.toBlob(res)));
    }
    keyZ.forEach(v =>{
        v.delete();
    })
